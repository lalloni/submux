...
// which supports Redis hashtag syntax: if the channel contains {tag},
// only the tag portion is used for hashslot calculation.
//
...
func Hashslot(channel string) int {
	...
	key := extractHashtag(channel)
	return int(crc16sum(key)) % 16384
}

// extractHashtag extracts the hashtag from a channel name.
// Redis hashtag syntax: {tag} - everything between first { and first }
// This matches go-redis/v9's internal/hashtag.Key function.
func extractHashtag(key string) string {
	// This matches the implementation from go-redis/v9/internal/hashtag/hashtag.go
...
// This matches go-redis/v9's internal/hashtag.crc16sum function exactly.
