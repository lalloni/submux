...
- **Per-subscription options**: `SubscribeSync`, `PSubscribeSync`, and `SSubscribeSync` now accept variadic `...SubscribeOption` for per-subscription configuration. Currently supports `WithQueueLimit`.
...
### Fixed
- **Stale PubSub reference during resubscription (TOCTOU race)**: `resubscribeOnNewNode` read `sub.getPubSub()` twice in separate loops. Between reads, a concurrent goroutine could call `setPubSub()`, causing `removeSubscription` to target the wrong metadata and orphan the subscription. Merged the two loops into one and snapshot the PubSub reference once per subscription. Fixes #6.
- **Direct PubSub connections now inherit ClusterClient configuration**: `createPubSubToNode` previously created direct Redis clients with only the node address, discarding authentication (Username, Password, CredentialsProvider, StreamingCredentialsProvider), TLS, dialer, timeouts, and protocol settings from the ClusterClient. This broke PubSub on authenticated or TLS-enabled clusters. Both `createPubSubToNode` and the topology monitor fallback now use a shared `clusterOptionsToNodeOptions` helper to cascade all relevant ClusterOptions fields. See ADR-003.
- **Failed PubSub connections now cleaned up automatically**: When an event loop exits due to a connection error, the `directClient`, PubSub, and pool map entries were never cleaned up until `SubMux.Close()`. For long-lived `SubMux` instances with intermittent connection failures, this leaked Redis TCP connections and memory. Added `onEventLoopExit` callback that fires after the event loop goroutine fully exits, closing all resources, removing from pool maps, and cleaning zombie subscriptions from `sm.subscriptions` when `autoResubscribe` is off.
